...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        da1, grads['W2'], grads['b2'] = affine_backward(ds, cache3)
        dz1 = relu_backward(da1, cache2)
        dx, grads['W1'], grads['b1'] = affine_backward(dz1, cache1)

        # add regularization
        loss += 0.5 * self.reg * np.sum(np.diagonal(self.params['W1'].T.dot(\
            self.params['W1'])))
        loss += 0.5 * self.reg * np.sum(np.diagonal(self.params['W2'].T.dot(\
            self.params['W2'])))
