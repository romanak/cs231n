...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss, dz[self.num_layers+1] = softmax_loss(scores, y)
        da, dz = {}

        for i in range(self.num_layers, 2, -1):
            da[i-1], grads[f"W{i}"], grads[f"b{i}"] = affine_backward(dz[i+1], \
                cache[i])
            dz[i] = relu_backward(da[i], z[i])
        
        dx, grads['W1'], grads['b1'] = affine_backward(dz[1], cache[1])

        # add regularization
        for i in range(1, self.num_layers):
            loss += 0.5 * self.reg * np.sum(np.diagonal(self.params[f"W{i}"].T \
                .dot(self.params[f"W{i}"])))
        for i in range(1, self.num_layers):
